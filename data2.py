...
def spectras_matrix(learning_sample, wavelengths, E_df, R_df, patches_number, illuminants_number=1):
    C = np.zeros(shape=(len(learning_sample), len(wavelengths)))
    C_current_index = 0
    R=R_df
    E_df = np.transpose(E_df)

    for illuminant_index in range(illuminants_number):
        
        E_interpolated=E_df
        E = np.diag(E_interpolated)
        ...
        print(R_learning)
        exit()
        C[C_current_index:C_current_index + len(R_learning)] = np.transpose(E @ R_learning)
...
def reflectances_matrix(R_df, R_wavelengths, patches_number, wavelengths):
