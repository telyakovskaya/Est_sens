...
def check_optimal(radiances, sensitivities, sensitivities_given, min_norm_difference_before_reg,
                  optimal_wavelength_number_before_reg):
    # colors_check = radiances[channel] @ sensitivities[channel]
    current_norm_difference = np.linalg.norm((sensitivities[channel] - sensitivities_given[channel]), 2)
...
learning_sample = choose_learning_sample(valid, ratio=1., patches_number=patches_number, illuminants_number=1,
...
for wavelengths_number in tqdm(range(2, 100)):
    ...
    for channel in range(3):
        ...
        min_norm_dif_before_reg[channel], optimal_wavelength_number_before_reg[channel] = \
            check_optimal(radiances, sensitivities, sensitivities_given[channel], min_norm_dif_before_reg,
                          optimal_wavelength_number_before_reg)
        ...
        min_norm_dif_reg_Tikhonov[channel], optimal_wavelength_number_reg_Tikhonov[channel] = \
            check_optimal(radiances, reg_sensitivities, sensitivities_given[channel], min_norm_dif_reg_Tikhonov,
                          optimal_wavelength_number_reg_Tikhonov)

        reg_sensitivities[channel] = regularization_derivatives(channel, wavelengths, radiances[channel],
                                                                patches_channel)
        reg_sensitivities[channel][reg_sensitivities[channel] < 0] = 0
        min_norm_dif_reg_derivatives[channel], optimal_wavelength_number_reg_derivatives[channel] = \
            check_optimal(radiances, reg_sensitivities, sensitivities_given[channel], min_norm_dif_reg_derivatives,
                          optimal_wavelength_number_reg_derivatives)
        ...
        # print(np.linalg.det(radiances[channel] @ np.transpose(radiances[channel])))
...
radiances = get_radiances(optimal_wavelength_number_before_reg, E_arr, E_wavelengths, R_arr, R_wavelengths,
                          patches_number)
for channel in range(3):
    patches_channel = np.array([patches_colors_filtered[channel][patch] for patch in learning_sample[channel]])
    sensitivities[channel] = inv((radiances[channel].T @ radiances[channel]).astype(float)) @ \
...
radiances = get_radiances(optimal_wavelength_number_reg_Tikhonov, E_arr, E_wavelengths, R_arr, R_wavelengths,
                          patches_number)
for channel in range(3):
...
radiances = get_radiances(optimal_wavelength_number_reg_derivatives, E_arr, E_wavelengths, R_arr, R_wavelengths,
                          patches_number)
for channel in range(3):
